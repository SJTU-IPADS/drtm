	...
	if (msg != "") {
	  ...
	  if(live_worker_num == 0) {
		...
		//end for
	      ...
	      fprintf(stdout,"cross warehouse: %d \n",total_cross_warehouse);
	      fprintf(stdout,"cross partition: %d \n",total_cross_partition);
...
tpcc_worker::txn_new_order()
{
  ...
  RTMTX::Begin(spinlocks,numW,&district_prof);
...
tpcc_worker::txn_payment()
{
 ...
 HERE:
  ...
  aborting = false;
  RTMTX::RdmaBegin(&store->payment_lock[customerWarehouseID - 1],&customerP_prof,&aborting);
  //RTMTX::Begin(&store->payment_lock[customerWarehouseID - 1],&customerP_prof);
...
tpcc_worker::txn_stock_level_rtm()
{
  ...
  spinlocks[0] = &store->neworder_lock[warehouse_id];
  ...
  while(1) {
    ...
    for(size_t time = 0;time < 2;++time){
      ...
      //      RTMTX::RdmaBegin(NULL,0,&readonly_prof);
      //      RTMTX::Begin(spinlocks,1,&readonly_prof);
      RAWRTMTX::Begin();
